...
def main(args):
    ...
    #    if overal_sims>-1:
        ...
        tmp_channel_b = [pb.get_random_rayleigh_variable(rayleigh_var, N, K)] # will return a matrix of K times N.
        tmp_channel = [pb.get_random_rayleigh_variable(rayleigh_var, N)] # Just to initialze this array.
        for k in range(N):
            tmp_channel[0][k,:] = tmp_channel_b[0][cell_mapping[:,0],k]
        tmp_H_all = np.multiply(np.sqrt(gains[0][:,:,0]),((abs(tmp_channel[-1]))))
        ...
        for i in range(1,total_samples):      
            ...
            if train_episodes is not None and i % train_episodes['T_train'] == 0:
                tmp_tmp_channel_b = pb.get_random_rayleigh_variable(rayleigh_var, N, K)
            else:
                tmp_tmp_channel_b = pb.get_markov_rayleigh_variable(tmp_channel_b[-1],correlation,rayleigh_var, N, K)
            ...
            tmp_H_all = np.multiply(np.sqrt(gains[0][:,:,i]),(((abs(tmp_tmp_channel)))))
            H_all.append(tmp_H_all)
